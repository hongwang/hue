...
from notebook.connectors.doris_type import parse_sqltype
...
class DorisApi(Api):
  ...
  def __init__(self, user, interpreter):
    super(DorisApi, self).__init__(user=user, interpreter=interpreter)
  ...
  def _create_engine(self):
    if '${' in self.options['url']:  # URL parameters substitution
      ...
      if '${PASSWORD}' in self.options['url']:
        ...
        userprofile = get_profile(self.user)
        if userprofile:
          token = userprofile.data.get('token')
          if token:
            vars['PASSWORD'] = decrypt_token(token)
            auth_provided = True
  ...
  def _get_columns(self, database, table):
    full_name = "{}.{}".format(database, table)

    columns = []
    engine = self._get_engine()
    with self._create_connection(engine) as conn:
      # SHOW COLUMNS returns: Field(0), Type(1), Null(2), Key(3), Default(4), Extra(5)
      result = conn.execute("SHOW COLUMNS FROM %s" % full_name)
      for row in result:
        column = dict(
            name=row[0],
            type=parse_sqltype(row[1]),
            nullable=row[2] == "YES",
            default=row[4],
        )
  ...
  def _show_tables(self, database):
    ...
    engine = self._get_engine()
    with self._create_connection(engine) as conn:
      result = conn.execute("SHOW FULL TABLES FROM %s" % database)
      for row in result:
        t = 'Table' if row[1] == "BASE TABLE" else 'View'
        tables.append({'name': row[0], 'type': t, 'comment': ''})
    return tables
  ...
  def _show_databases(self):
    ...
    with self._create_connection(engine) as conn:
      for catalog in catalogs:
        try:
          result = conn.execute('SHOW SCHEMAS FROM ' + catalog)
          databases += [f'{catalog}.{row["Database"]}' for row in result]
        except Exception as e:
  ...
  def _show_catalogs(self):
    ...
    with self._create_connection(engine) as conn:
      ...
      catalogs = [row['CatalogName'] for row in result]
      return catalogs
  ...
  def get_sample_data(self, snippet, database=None, table=None, column=None, is_async=False, operation=None):
    ...
    metadata, sample_data = self._get_sample_data(database, table, column=column, operation=operation)
  ...
  def get_browse_query(self, snippet, database, table, partition_spec=None):
    return textwrap.dedent('''\
      SELECT *
      FROM %(database)s.%(backticks)s%(table)s%(backticks)s
      LIMIT 1000
      ''' % {
        'database': database,
        'table': table,
  ...
  def _get_sample_data(self, database, table, column=None, operation=None):
    if operation == 'hello':
